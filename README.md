...
| 用途 | 模型 | 說明 | 硬體 / VRAM |
|------|------|------|-------------|
| 圖片生成 <br>（雲端） | **Google Gemini** <br>（`gemini-3-pro-image-preview`） | 依提示詞與參考圖生成 4K、16:9 圖片 <br> 需 `GOOGLE_API_KEY`。 | 不需本地 GPU。 |
| 圖片生成 <br>（本地） | **FLUX.2-klein** <br>（Black Forest Labs） | 需本地 GPU 依顯存自動選擇模型。 | **Flux 4B** 至少 8GB VRAM；<br> **Flux 9B** 至少 20GB VRAM。 |
